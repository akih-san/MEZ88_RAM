...
# 2 "<built-in>" 2
# 1 "../src/mez88_47Q_main.c" 2
# 24 "../src/mez88_47Q_main.c"
...
void main(void)
{
 ...
 printf("\n\r");
    ...
    printf("Use PWM1  %.2fMHz(Duty %.1f%%) for 8088 clock\n\r",
       1/((PWM1PR+1)*15.625)*1000, (double)(PWM1S1P1*100/(PWM1PR+1)));
    printf("\n\r");
...
static void setup_msdos(void) {
 ...
 flg = load_program(buf, 0x7f500, &size);
 if (!flg) {
  sprintf((char *)buf, "%s/IO.SYS", fileinfo.fname);
  flg = load_program(buf, 0x400, &size);
  if (!flg) {
   ld_adr = (((size+sec_size) / sec_size)+1)*sec_size+0x400;
...
static void setup_cpm(void) {
 ...
 if (!flg) {
  ...
  flg = load_program(buf, 0x400 + 0x2500, &size);
  if (!flg) {
...
static int load_program(const TCHAR *buf, uint32_t load_adr, uint16_t *load_size) {
 ...
 adr = load_adr;
 cnt = *load_size = (uint16_t)((&rom_fl)->obj.objsize);
 btr = 256 * 2;
 while( cnt ) {
...
static int chk_os(void)
{
 ...
 while (f_readdir(&fsdir, &fileinfo) == FR_OK && fileinfo.fname[0] != 0) {
  if (strcmp(fileinfo.fname, cpmdir) == 0) selection |= 1;
  if (strcmp(fileinfo.fname, msdosdir) == 0) selection |= 2;
 ...
 switch( selection) {
  ...
  default:
         ...
         printf("\n\r");
  if ( selection == 1) strcpy(fileinfo.fname, cpmdir);
...
static int open_dskimg(void) {
 ...
 for (drv = num_files = 0; drv < 16 && num_files < 6; drv++) {
    ...
    if (dos_drives[0].filep == ((void*)0)) return -4;
         }
         num_files++;
        }
    }
