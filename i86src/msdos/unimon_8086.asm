...
FREE_SEG	equ	DOS_SEG
BIOS_IP		equ	IOSYS_OFF
...
int224vec:	equ	0E0h * 4 ; INT 0E0H Vector table offset
;;;
;;; Options
;;;
REQ_CONIN	equ	1
...
MNI_LEDOFF	equ	05h	; NMI signal off request command



;-------------------------------------------------------
; sumple writing
;	mov	bx, REQH_CMD	; = offset REQH.CMD
;	mov	al, [REQH_CMD]	; = byte ptr REQH.CMD
;-------------------------------------------------------
...
;          = 4 ; unimon request STROUT : string address = (PTRSAV, PTRSAV_SEG)
...
; ------------------OFFSET = 6
DREQ_COM	db	0		; Device  request command
DEV_RES		db	0		; reserve
; ------------------OFFSET = 8
PTRSAV		dw	0		; string offset
PTRSAV_SEG	dw	0		; string segment

	db	MON_B - $ dup(0)	; reserve
...
IR0:
	...
	mov	ax, DOS_SEG	; set free memory start segment
...
go_anywhere:
	mov	cs:[UREQ_COM], MNI_LEDOFF	; NMI LED off request
...
stout_req:
	out	DUMMY_PORT, al		; invoke PIC F/W
